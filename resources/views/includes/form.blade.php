...
<div class="mb-3">
    ...
    <input value="{{ old('title', $project->title) }}" type="text" class="form-control" id="title"
        aria-describedby="titleHelp" name="title" required>
    <div class="form-text">Required</div>
</div>
<div class="mb-3">
    <label for="slug" class="form-label">Repository name</label>
    <input disabled value="{{ Str::slug(old('title', $project->title)) }}" type="text" class="form-control"
        id="slug" aria-describedby="slugHelp" name="slug" required>
...
<div class="mb-3">
    ...
    <input value="{{ old('image', $project->image) }}" type="url" class="form-control" id="image"
        name="image">
</div>
<div class="row">
    <div class="mb-3 col-6">
        <label for="main_lang" class="form-label">Main language used</label>
        <input value="{{ old('main_lang', $project->main_lang) }}" type="text" class="form-control" id="main_lang"
            name="main_lang">
    </div>
    <div class="mb-3 col-6">
        <label for="other_langs" class="form-label">Other languages used</label>
        <input value="{{ old('other_langs', $project->other_langs) }}" type="text" class="form-control"
            id="other_langs" name="other_langs">
    </div>
</div>
<div class="mb-3 w-25">
    <label for="n_stars" class="form-label">Number of stars recived</label>
    <input value="{{ old('n_stars', $project->n_stars) }}" type="number" class="form-control" id="n_stars"
        name="n_stars">
</div>
<div class="mb-3 form-check">
    <label class="form-check-label" for="is_public">Open source</label>
    <input type="checkbox" @if (old('is_public', $project->is_public)) checked @endif value="1" class="form-check-input"
